...
// ==================== Generic Net Proxy（绕过浏览器 CORS） ====================
// 渲染进程直连某些第三方 API（如 ark.cn-beijing.volces.com 火山方舟原生域）会因
// 服务端不返回 Access-Control-Allow-Origin 头被 Chromium 拦截抛 "Failed to fetch"。
// 主进程 net.fetch 不受 CORS 限制，统一在此代理。
type NetProxyRequest = {
  url: string
  method?: string
  headers?: Record<string, string>
  /** body 只接受字符串或 base64（用于二进制） */
  body?: string
...
ipcMain.handle('net:proxy-fetch', async (_event, req: NetProxyRequest): Promise<NetProxyResponse> => {
  if (!req?.url || typeof req.url !== 'string') {
    throw new Error('net:proxy-fetch 缺少 url')
  }
  const controller = new AbortController()
  const timeout = Math.max(1000, Math.min(req.timeoutMs ?? 120_000, 10 * 60_000))
