...
export const useFreedomStore = create<FreedomStore>()(
  persist(
    (set, get) => ({
      ...
      setUploadProgress: (id, entry) => set((state) => {
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // 注意：imageReferenceImages 是 dataURL 数组（可能很大），
        // 不持久化到 localStorage，避免每次 setState 都触发巨型序列化导致主线程卡死。
        // 切换 Tab 时仍保留在内存中（zustand 的运行时状态）。
